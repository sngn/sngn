...
/** @type {DependencyType[]} */
const dependencyTypes_to_mutate = ["dependencies", "devDependencies", "peerDependencies"];
...
function replaceVersions(workspaceVersionMap) {
  workspaceVersionMap.forEach((lv /*, pkg*/) => {
    ...
    writeFileSync(`${location}/package.json`, data, "utf-8");
  });
}
...
function generateWorkspacesDefinitionsPnpm(workspaceConfigPath) {
  ...
  try {
    ...
    const error = new Error(
      `Could not read workspaces configuration file '${workspaceConfigPath}'`,
      { cause },
    );
    console.error(error);
